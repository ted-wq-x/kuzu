...
#include "function/schema/table_id_functions.h"
...
static void TableIDExecFunc(const std::vector<std::shared_ptr<ValueVector>>& params,
    ValueVector& result, void* /*dataPtr*/ = nullptr) {
    KU_ASSERT(params.size() == 1);
    UnaryFunctionExecutor::execute<internalID_t, int64_t, TableID>(*params[0], result);
}

function_set TableIDFunction::getFunctionSet() {
    function_set functionSet;
    functionSet.push_back(
        make_unique<ScalarFunction>(name, std::vector<LogicalTypeID>{LogicalTypeID::INTERNAL_ID},
            LogicalTypeID::INT64, TableIDExecFunc));
    return functionSet;
}
