...
struct GDSFunction : public Function {
    ...
    GDSFunction(std::string name, std::unique_ptr<GDSAlgorithm> gds)
        : Function{std::move(name), gds->getParamTypeIDs()}, gds{std::move(gds)} {}
    GDSFunction(const GDSFunction& other)
        : Function{other}, gds{other.gds->copy()} {}

    std::unique_ptr<Function> copy() const override {
        return std::make_unique<GDSFunction>(*this);
    }
};
