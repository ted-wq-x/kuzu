...
ChunkedCSRHeader::ChunkedCSRHeader(bool enableCompression, uint64_t capacity) {
    offset = std::shared_ptr<ColumnChunkData>(ColumnChunkFactory::createColumnChunkData(
        LogicalType::UINT64(), enableCompression, capacity));
    length = std::shared_ptr<ColumnChunkData>(ColumnChunkFactory::createColumnChunkData(
        LogicalType::UINT64(), enableCompression, capacity));
}
